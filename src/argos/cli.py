...
async def _run(dynamic_urls: list[str] | None) -> int:
    ...
    if summary.preflight_filtered > 0:
        print(f"사전 필터 제거: {summary.preflight_filtered}개 (명백한 비기술 콘텐츠)")
    if summary.trust_skipped > 0:
        print(f"신뢰도 낮음 스킵: {summary.trust_skipped}개 (genealogy 생략)")
