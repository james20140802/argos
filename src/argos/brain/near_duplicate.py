...
from argos.brain.entity_names import MARK_CLASS, opens_a_position
...
def _keep_name_symbols(match: re.Match[str]) -> str:
    ...
    기사가 똑같아진다. 이름이 시작할 수 있는 자리(글머리·공백·여는 구두점)에
    놓인 점만이다. 판정은 고유명사 쪽 토큰화와 같은 것을 쓴다 — 갈라 두면 같은
    글이 두 경로에서 다르게 잘린다.
    ...
    lead = "." if run.endswith(".") and after.isalpha() and opens_a_position(ahead) else ""
