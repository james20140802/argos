...
# 낱말 안에서 이름을 잇는 부호. 가운뎃점(U+00B7·U+2027)도 넣는다 — 'DALL·E'는
# 거기서 끊으면 제품 하나가 사라지고 조각 둘이 남는다. 목록을 나누는 가운뎃점은
# 보통 앞뒤에 공백이 있어서 안 걸린다.
_JOIN = r"[-'’./·‧‐-–−]"
# 갈래 셋을 이 순서로 본다.
...
_TOKEN = re.compile(
    rf"\.?[^\W\d_]{_WORD}*(?:{_JOIN}{_WORD}+)*[+#]*"
    rf"|\d+(?:\.\d+)*(?![^\W_]|{_JOIN}[^\W_])"
    rf"|\d+{_WORD}*(?:{_JOIN}{_WORD}+)*[+#]*"
)
