...
_JOINERS = frozenset({"&", "＆"})
# 이름의 꼬리. 앞에 쉼표를 두고 쓰는 관례가 있어('Acme, Inc.', 'Foo Co., Ltd.',
# 'Martin Luther King, Jr.'), 쉼표에서 끊으면 이름 하나가 조각 둘로 남고 쉼표
# 없이 쓴 같은 이름과 다른 키가 된다. 쉼표로 나열한 이름들과는 **뒤에 오는
# 낱말**로 가른다 — 이 목록에 든 것만 이어 붙인다.
...
# 괄호와 대괄호도 여기 든다. 여닫는 모양이 다르고, 안이 문장인지 이름인지는
# 아래 `_quotes_a_clause`가 따옴표와 똑같이 가른다 — 'Acme Corp (Globex)'의
# 이름은 남고 '[Customers pay monthly.]'는 새 문장의 첫 자리가 된다.
...
def _joins_a_name_suffix(gap: str, token: str) -> bool:
    ...
    return (
        gap.replace(".", "") == ","
        and canonical_name(token) in _NAME_SUFFIXES
    )
...
def _candidates(document: str, max_ngram: int) -> list[_Candidate]:
    ...
    for sentence_offset, sentence in _sentences(normalized):
        ...
        for match in _TOKEN.finditer(sentence):
            ...
            # 이름 꼬리 앞의 쉼표도 예외다 — 'Acme, Inc.'와 'King, Jr.'는
            # 이름 하나다.
            if (
                run
                and gap
                ...
                and not _joins_a_name_suffix(gap, match.group())
            ):
                flush()
