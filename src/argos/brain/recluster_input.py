...
**비용은 기간 길이가 아니라 창 안 밀도가 정한다.** 안쪽 `WHERE`의 ±window_days
술어가 거리 계산 대상을 먼저 줄이므로, 문서당 768차원 거리를 재는 건 기간 전체가
아니라 그 문서의 시간 이웃뿐이다. 실측(2026-09-11, 전 코퍼스 1321건 / 18개월,
벡터 인덱스 없음): `EXPLAIN ANALYZE`가 안쪽에 `rows=125 loops=1321` — 1321이
아니라 125다. 전 기간을 한 번에 돌려도 쿼리 748ms, `fetch_period_input` 전체
1.12s(14일 0.18s · 3개월 0.64s). 그래서 기간 길이에 상한을 걸지 않았다.

남는 제곱 항은 거리가 아니라 CTE 행 방문이다(루프마다 `period`를 훑어 시간
술어를 적용). 지금 규모에서는 위 숫자에 묻히지만, 코퍼스가 10배쯤 되면 이 항이
먼저 아프다. 그때 손볼 자리는 인덱스이고, 그건 ARG-241에서 ANN을 보류하며
정리해 둔 결정이라 여기서 규모 논거만으로 되살리지 않는다."""
