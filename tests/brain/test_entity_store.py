...
def test_attach_extracted_names_loads_names_onto_valid_states_only():
    ...
    result = brain_pipeline._attach_extracted_names(
        [valid_state, invalid_state], extractor=extract_mock
    )
...
def test_attach_extracted_names_swallows_extraction_failure():
    """추출이 터져도 크롤을 멈추지 않는다 — 빈 목록으로 이어간다."""
    valid_state = _state()
    extract_mock = MagicMock(side_effect=RuntimeError("boom"))

    result = brain_pipeline._attach_extracted_names([valid_state], extractor=extract_mock)

    assert result[0]["entity_names"] == []
    assert result[0]["entity_names_extracted"] == []


def test_attach_extracted_names_no_valid_states_skips_extraction():
    """유효한 문서가 하나도 없으면 추출기를 아예 부르지 않는다."""
    invalid_state = _state(is_valid=False)
    extract_mock = MagicMock()

    result = brain_pipeline._attach_extracted_names([invalid_state], extractor=extract_mock)
