...
- **IMPORTANT:** The AI assistant writes files to the workspace folder. All git operations (commit, push, branch) are performed by the user on their local machine (`~/Documents/argos`). The assistant provides commit messages and git commands as text — never executes git commands directly.
